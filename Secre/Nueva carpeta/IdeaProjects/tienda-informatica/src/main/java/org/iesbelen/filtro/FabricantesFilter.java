...
public class FabricantesFilter extends HttpFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        String comparar = httpRequest.getRequestURI().substring(httpRequest.getContextPath().length());

        Usuario usuario = null;


        if (session != null  //Seteo inline de usuario
                && (usuario = (Usuario)session.getAttribute("usuarioLogin") )!= null
            ...
            return;
                ...
                || comparar.matches("/tienda/fabricantes/\\d+")) {
