...
public class ProductosServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pathInfo == null || "/".equals(pathInfo)) {
			...
			if (filtro != null) {
				listaProd = prodDAO.filtrar(filtro);
			}else {
				listaProd = prodDAO.getAll();
			}
			...
			if (pathParts.length == 2 && "crear".equals(pathParts[1])) {
				...
				// GET
				// /productos/crear
				FabricanteDAOImpl fab = new FabricanteDAOImpl();
				List<String> lista = fab.getAll().stream()
						.map(Fabricante::getNombre)
						.toList();

				request.setAttribute("fabricantesNombres", lista);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (__method__ == null) {
			...
			String idFab = request.getParameter("fabricante");

			Producto nuevoProd = new Producto();
			nuevoProd.setNombre(nombre);
			nuevoProd.setPrecio(Double.parseDouble(precio));
			nuevoProd.setCodigo_fabricante(Integer.parseInt(idFab));
